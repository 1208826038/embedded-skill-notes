...
> 本导读为仓库内 54 篇技术长文（技能梳理 20 篇 + 外设协议 14 篇 + BMS 进阶 12 篇 + 操作系统进阶 8 篇）设计了一条**由浅入深、理论→实战→面试**的学习路径。每篇均含「场景引入 → 核心原理 → 生动类比 → 代码/时序 → 常见坑 → 面试要点」，并已配 mermaid 示意图。
>
...
- **想补 OS 理论体系**：先读 `02-rtos-tuning.md`（RTOS 工程）→ **操作系统进阶(o01~o08：进程线程/调度/同步死锁/内存/中断系统调用/文件系统IO/嵌入式Linux/面试全景)**，把 RTOS 工程实践回填到经典 OS 理论；其中 o02 调度与 b09 AUTOSAR 优先级、o03 同步与 `02` 死锁章直接呼应。
