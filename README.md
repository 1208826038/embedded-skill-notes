...
| `uds.md` | **UDS 统一诊断服务深度解析（ISO 14229-1）**：10 节、中文 1770+ 字、含 5 张对比/参数表，系统讲透 UDS 定位与 OBD-II 区别、协议栈位置、请求/响应/负响应格式、核心服务全集（$10/$11/$14/$19/$22/$23/$24/$27/$28/$2E/$2F/$31/$3E/$85 及 $34/$36/$37 刷写四件套）、NRC 负响应码一览、会话/安全状态机与时序（P2/P2*/S3）、ISO-TP 分段（SF/FF/CF/FC）与 DoIP、AUTOSAR DCM/DEM/FiM 实现、典型刷写流程、工程坑与面试高频；既随 面试神器.html 内嵌检索，也可单独阅读 |
| `asw.md` | **AUTOSAR 应用层（ASW）模块深度解析**：12 节、中文 1590+ 字、含 4 张对比/参数表，系统讲透 ASW 在架构中的位置与「SWC 不经 RTE 不碰硬件」铁律、SWC 类型（Atomic 各子类/CDD/Composition）、端口 PPort/RPort 与四类接口（SenderReceiver/ClientServer/ModeSwitch/Parameter）、Runnable 与事件（Timing/DataReceived/OperationInvoked/ModeSwitch/Init）、RTE 生成代码与 API（Rte_Read/Write/Call/Mode/Enter-Exit）、同/跨 ECU 通信语义、Assembly/Delegation 连接器、NvM 与 SWC 衔接、标定 Parameter/XCP/A2L、数据类型三层体系（ADT/IDT/BaseType）、AUTOSAR 方法论、与 OS/BSW 协同、工程坑与面试高频；既随 面试神器.html 内嵌检索，也可单独阅读 |
...
- **知识学习**：内嵌十份文档（技能梳理 / 外设协议 / 进阶深挖 / C 语言基础 / AUTOSAR OS 深度解析 / E2E 通信保护深度解析 / Flash 与存储栈深度解析 / BMS 算法与 Simulink 建模 / UDS 诊断深度解析 / ASW 应用层深度解析），可搜索高亮、目录跳转。
...
[`S32K144_上手指南.md`](S32K144_上手指南.md) 是一份**从下单到跑通第一个 CAN 工程、再到做出能接单的 BMS 从板 demo** 的实操文档：含三档采购清单（2026-08 行情与渠道）、全免费软件工具链安装、四步递进上手指引（开箱→点灯 printf→AUTOSAR MCAL 跑 CAN→调试手法）、BMS 从板开源工程骨架，以及对接 Upwork / 程序员客栈等自由职业平台的路径。与 `面试神器.html`、`study-roadmap.md` 互为「硬件落地 / 题库 / 理论路线」三件套。
