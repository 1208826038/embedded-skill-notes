...
本仓库收集了笔者在学习与项目中沉淀的资料：**技能梳理文档、外设协议详解、离线模拟面试应用、55 篇技术长文（含示意图）、学习路线导读，以及一套可打印的电子书**。既可以作为复习资料，也可以作为技术作品集对外分享。工程脚本（Polyspace / QAC / DBC 转换）已独立存放于私有仓库 [`embedded-scripts`](https://github.com/1208826038/embedded-scripts)。
...
| `study-roadmap.md` | **文章学习路线导读**：把 55 篇按基础→进阶→专项分层，给出学习顺序、各阶段目标与自测建议 |
| `技术文章合集.html` | **电子书（HTML 版）**：55 篇文章 + 学习路线聚合为单文件，含目录侧栏，mermaid 示意图在线渲染，可浏览器直接阅读或打印为 PDF |
